...
LogLine::~LogLine()
{
	if (mCurrentLevel <= mSetLevel && mSetLevel > LogLevel::logDISABLE)
	{
		...
		cout << "\033[0m" /* end of color */ << mStream.str();
		cout.flush();
	}
}
...
void LogLine::putHeader(const string& header)
{
	if (mCurrentLevel <= mSetLevel && mSetLevel > LogLevel::logDISABLE)
	{
		...
		mStream << levelToColor(mCurrentLevel) << nowTime()
				<< " | " << header << " "
...
string LogLine::levelToColor(LogLevel level)
{
	static const char* buffer[] = {"", "\033[31m", "\033[33m", "\033[32m", "\033[37m"};
